  ...
  return (
    <div className="p-4 max-w-4xl mx-auto bg-white rounded-lg shadow-md">
      ...
      {story.quote.length > 0 &&
        ...
        <blockquote className="bg-gray-100 p-4 mb-4 border-l-4 border-gray-300 poppins-regular-italic">{story.quote}</blockquote>
      }
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4 poppins-regular ">
        {story?.images.map((src, i) => (
          <div key={i} className="relative h-64 w-full">
            ...
            />
            <p className="absolute bottom-0 bg-black bg-opacity-50 poppins-regular text-white text-xs p-2 rounded-b-md w-full">{story.imgCaption[i]}</p>
          </div>
        ))}
      </div>
      <div className="text-gray-800 space-y-4 poppins-regular text-sm">
